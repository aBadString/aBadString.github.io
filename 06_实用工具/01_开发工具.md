...
- [7. Ubuntu](#7-ubuntu)
  ...
  - [7.6. 文件上传](#76-文件上传)
  - [7.7. 常见问题](#77-常见问题)
  - [7.8. 磁盘空间不足](#78-磁盘空间不足)
  - [7.9. 文件目录结构](#79-文件目录结构)
- [8. CMD](#8-cmd)
...
1. VMware Authonrization Service: 用于启动和访问虚拟机的授权和身份验证服务
2. VMware DHCP Service: IP 自动分配协议
3. VMware NAT Service: 虚拟地址转换协议
4. VMware USB Arbitration Service: U 盘接口服务
5. VMware Workstation Server: 用于虚拟机的注册和管理远程访问服务

NAT 模式下 DHCP 负责给物理机的 v8 网卡、虚拟机的网卡分配 IP。**如果关闭了 DHCP 服务，则物理机和虚拟机的网卡全部要自动分配 IP**

> 在默认配置中，虚拟 DHCP 服务器动态分配 net.128 到 net.254 范围内的 IP 地址，其中 net 是为 NAT 模式网络分配的网络编号。Workstation Pro 始终在 NAT 模式网络中使用 C 类地址。可以将网络.3 到 网络.127 之间的 IP 地址用于静态 IP 地址。IP 地址“net.1”是为主机虚拟网络适配器预留的，net.2 是为 NAT 设备预留的。
...
1. 首先将 VM 虚拟机设置 网络适配器 改为 NAT模式

2. 在主机(Win 10)上查看 VMware Network Adapter VMnet8 网卡的IP

控制面板\网络和 Internet\网络和共享中心 -> 更改适配器控制 -> 右键状态 -> 详细信息
![](../images/VMware的v8网卡.png)

==**192.168.18.2 这个ip 就是虚拟机(Ubuntu)配置静态 IP 时的网关。**==
...
network:
  ethernets:
    ens33:
      dhcp4: false                  # 关闭动态分配 IPv4 地址
-     addresses: [192.168.1.100/24] # 本机固定 IP 和掩码
+     addresses: [192.168.18.100/24] # IP 和子网掩码需要修改，使得网络号和网关保持一致 (此时网络号为 192.168.18.0)
-     gateway4: 192.168.1.1         # 网关(局域网的路由器 IP)
+     gateway4: 192.168.18.2        # 网关修改为第2步查看的 IP
      nameservers:
-       addresses: [192.168.1.1,119.29.29.29]    # DNS 服务器地址 192.168.1.1是局域网内的DNS 119.29.29.29是公网DNS
+       addresses: [192.168.18.2,202.101.224.69,119.29.29.29]
  version: 2
```
...
```shell
> ip route show
default via 192.168.18.2 dev ens33 proto static
172.17.0.0/16 dev docker0 proto kernel scope link src 172.17.0.1
192.168.18.0/24 dev ens33 proto kernel scope link src 192.168.18.100
```
可以看到现在网关已经是 192.168.18.2，并且虚拟机的 IP 已经改成 192.168.18.100 了。
...
1. 首先查看磁盘使用情况
```shell
abadstring@ubuntu:~$ df -hl
Filesystem      Size  Used Avail Use% Mounted on
/dev/sda2        20G   19G  539M  98% /
...
2. 定位大文件目录
```shell
abadstring@ubuntu:~$ cd /
abadstring@ubuntu:/$ sudo du -h --max-depth=1 | sort -h
0       ./dev
0       ./proc
0       ./sys
4.0K    ./cdrom
...
一路寻找发现 /var/lib/docker/overlay2 占了 8G 空间
```shell
abadstring@ubuntu:/var/lib/docker$ sudo du -h --max-depth=1 | sort -h
4.0K    ./runtimes
4.0K    ./swarm
4.0K    ./tmp
4.0K    ./trust
...
原来是 Docker 占了很大空间，查看 Docker 的磁盘占用情况
```shell
abadstring@ubuntu:/var/lib/docker$ sudo docker system df
TYPE            TOTAL     ACTIVE    SIZE      RECLAIMABLE
Images          17        13        6.916GB   1.709GB (24%)
Containers      16        1         2.604GB   2.604GB (99%)
Local Volumes   28        10        846.9MB   32.77kB (0%)
...
```shell
abadstring@ubuntu:/var/lib/docker$ sudo docker images
REPOSITORY                                                      TAG              IMAGE ID       CREATED         SIZE
nacos/nacos-server                                              2.0.2            c8cd99bb135c   4 weeks ago     1.05GB
zookeeper                                                       latest           2bd79ece4af2   2 months ago    269MB
foxiswho/rocketmq                                               4.8.0            98bc46e1c0c1   6 months ago    505MB
jeanblanchard/alpine-glibc                                      latest           4fad0894f97c   7 months ago    17.7MB
redis                                                           5.0.5            63130206b0fa   22 months ago   98.2MB
percona                                                         5.7.23           9db94bcf28b3   2 years ago     577MB
mongo                                                           4.0.3            05b3651ee24e   2 years ago     382MB
styletang/rocketmq-console-ng                                   1.0.0            7df83bb6e638   3 years ago     702MB
# 删除了
abadstring/md-doc-repo-service                                  1.0.0            0b0cd8184861   7 months ago    177MB
abadstring/md-doc-repo-service                                  latest           0b0cd8184861   7 months ago    177MB
abadstring/jre8u275                                             latest           9dfb75b25ecf   7 months ago    129MB
consul                                                          1.9.7            a575ae275523   5 weeks ago     123MB
tomcat                                                          latest           2ae23eb477aa   12 months ago   647MB
tomcat                                                          8.0.52           b4b762737ed4   3 years ago     356MB
mysql                                                           8.0.15           7bb2586065cd   2 years ago     477MB
redis                                                           latest           16ecd2772934   9 months ago    104MB
registry.cn-hangzhou.aliyuncs.com/aliacs-app-catalog/rabbitmq   3.7.8-debian-9   1f174f0c5769   2 years ago     228MB
registry.cn-hangzhou.aliyuncs.com/lab99/gitlab-ce-zh            latest           3884a8178ad9   4 years ago     1.22GB
```
...
/
# 系统目录
|-- boot: 启动 Linux 的核心文件, 连接文件和镜像文件
|-- etc: 系统配置文件
# 虚拟目录
|-- proc: 是系统内存的映射. 访问这个目录来获取系统信息
|-- dev: Device(设备). 外部设备文件
|-- sys: 用于将系统中的设备组织成层次结构, 是内核设备树的一个直观反映
# 用户
|-- home: 用户目录
|-- root: root 用户目录
# 命令和库
...
|-- lib -> usr/lib: 共享的动态链接库（lib32, lib64, libx32）
# 应用程序
|-- usr: unix shared resources(共享资源). 存放应用程序和文件(Program Files)
    # 以下三个映射到根目录
    |-- bin
    |-- sbin
    |-- lib
    ...
    |-- local
        ...
        |-- etc
# var
|-- var: variable(变量). 存放着在不断扩充着的东西, 如日志文件
    |-- log: 存放日志
    |-- run -> /run
    |-- lock -> /run/lock
    |-- lib
    |-- local
    |-- spool: SPOOLing 技术, 假脱机真联机
# 临时文件目录
|-- run: 临时目录, 存储系统启动以来的信息, 当系统重启时删除
|-- tmp: temporary(临时). 存放临时文件
|-- lost+found: 一般情况下是空的. 当系统非法关机后, 这里就存放了一些文件
# 挂载目录, 不挂载为空
|-- media: 自动挂载的目录. U盘, 光驱等
|-- mnt: 手动挂载一些临时媒体设备的目录
# 其他
|-- opt: 自己安装软件的目录 (默认空), 另一个自己安装软件的目录是 /usr/local
|-- srv: 服务启动之后需要提取的数据(不用服务器就是空). 如 web 服务器就将 HTML 放到 /srv/www, ftp 服务器就放到 /src/ftp
```
...
|-- var/log: 存放日志
|-- tmp: temporary(临时). 存放临时文件
|-- opt: 自己安装软件的目录, 可放置大型软件. 可挂载至其他磁盘
|-- srv: 服务器文件
```
